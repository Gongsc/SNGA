...
站点管签到叫「每日登录奖励」，页面是 `/mission/daily`。它没有接口，领取是一颗
`input[type=button]` 的 `onclick`（`location.href = '…'`）—— 和收藏一样，
**地址不拼，读出来**，令牌就在里面。

按钮的形状是实测的：同一页上「查看我的账户余额」那颗就是
`onclick="location.href = '/balance';"`。所以同一页上不止一颗按钮，靠**路径**分 ——
领奖那条在 `/mission/` 底下。认路径而不是认按钮上的字：那句字会随奖励金额变
（「领取 20 铜币」），路径不会。

状态也在这一页上：站点写着「已连续登录 8 天」（实测），而「今天领了没有」看的是
那颗领奖按钮还在不在 —— 领过之后它被换成「查看我的账户余额」。**总天数站点不报**，
`CheckInStatistics.totalDays` 留 nil，界面就不显示那一行；填 0 是在说一件错事。
