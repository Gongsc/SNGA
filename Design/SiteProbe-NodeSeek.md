...
- 投票（2026-08-27 摸到一半）：
  ...
  - 提交是 `POST /api/vote/voteforitem`，请求体 `{"ids":[选项编号]}`，
    响应只有 `{success: boolean}`。**请求体里没有投票编号** —— 选项编号本身
    就是全局唯一的，站点靠它反查是哪个投票。多选就是往同一个数组里多放几个。
  - 匿名下 `URLSession` 能不能取到投票没验过（403 的原因还没查清）。取不到时
    按「这帖没有投票」处理，不让一个附加内容把整页帖子拖垮。
  - `.poll` 已点亮（2026-08-27）。界面上「本站的投票暂时只能查看」那条分支保留 ——
    别的站点用得上，而且它和「投票已结束」不是一回事。
