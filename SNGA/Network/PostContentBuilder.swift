...
enum PostContentBuilder {
    ...
    /// 原生渲染的规模上限。
    ///
    /// 楼层和楼内的块都是一次性实例化的，一页的视图规模就是各层块数之和。实测单层
    /// 的首次布局和每次重排：200 块是 117 ms / 34 ms，800 块是 1.1 s / 74 ms，
    /// 2000 块是 6.7 s / 383 ms；引用嵌套 120 层是 526 ms / 43 ms。布局每个显示周期
    /// 都要跑一遍，到这个量级主线程就再也追不上，界面直接卡死。
    ///
