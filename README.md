...
SNGA 是一个面向 macOS 26 的原生 NGA 论坛客户端，当前版本为 1.6.1。项目初衷是为了方便上班摸鱼，后续会根据使用情况继续完善。
...
- 主题列表增加“最新回复”和“最新话题”排序，并支持进入精华区
- 主题列表工具栏增加置顶话题入口，可直接在详情区域查看版面置顶内容
- 主题详情增加“只看作者”，回复工具栏可在全部楼层与楼主回复之间切换
- 每个楼层左下角显示 Apple、Android 或桌面设备图标
- 兼容 NGA 当前提醒消息的脚本封装格式，修复论坛回复提醒缺失
- 正确识别帖子锁定响应，并在打开或尝试回复锁帖时显示明确提示
- 统一主题列表与主题详情的刷新图标，并修正小工具空内容或加载失败时的底部工具栏位置
...
- 切换收藏板块时自动刷新主题列表、回到顶部并显示骨架屏动画
- 支持主板块与子板块，子板块列表可折叠
- 第一次进入板块时使用网页默认的子板块选择，之后保存每个账号的选择
- 主板块主题列表可标识主题所属子板块，点击子板块可进入对应列表
- 正确处理版面镜像主题并跳转到目标板块
- 主题列表支持分页、首页/尾页和指定页跳转
- 主题列表支持按最新回复或最新话题排序，并可切换只显示精华主题
- 主题列表工具栏支持直接打开当前版面的置顶话题
- 主题列表支持一键回到顶部
- 底部翻页与操作栏支持自适应紧凑布局，操作按钮固定在右侧
- 底部按钮提供鼠标悬停高亮和按下反馈
- 支持鼠标悬停与当前主题选中高亮，发帖时间使用静态格式
- 支持 NGA 主题标题颜色，并自动修复部分列表接口产生的 UTF-8/GB18030 乱码

### 收藏夹

- 同步 NGA 账号的主题收藏夹，并按收藏目录分页浏览主题
- 可新建、重命名和删除收藏夹，设置公开状态及默认收藏夹
- 可从主题底部菜单将主题加入一个或多个收藏夹，或一次取消全部收藏
- 收藏夹显示主题数量，并支持从当前目录快速移除主题
- 板块收藏与主题收藏夹相互独立，均按账号隔离

### 主题浏览

- 主题列表与主题内容保持分栏浏览
- 主题列表和主题内容在加载、刷新及翻页时使用骨架屏动画
- 支持帖子分页、首页/尾页和指定页跳转
- 支持只看主题作者，并使用筛选结果的独立分页信息
- 支持一键回到主题内容顶部
- 展示主题标题、楼层、作者、头像、发帖时间、发帖设备及引用关系
- 点击用户名或头像可打开用户中心，点击楼层引用可跳转到目标楼层
- 正文中的 NGA 主题、楼层、版面和用户链接会在应用内打开；关联主题跳转与多级返回使用骨架屏过渡动画
- 支持热点回复，并在楼主与普通回复之间使用不同背景显示
- 支持楼层点赞、点踩及数量展示
- 支持主题投票的单选、多选、分组、截止时间、结果展示及站端提交
- 支持主题评分汇总、回复评分展示，并可在回复时选择评分一并提交
...
- 板块、子板块、置顶话题、镜像主题、主题颜色、排序、精华筛选、列表乱码修复、楼层设备、热点回复和用户资料解析
- 全部版面搜索与小工具 API、热榜、实例回退及自定义地址解析
- 富文本、列表、折叠内容、表情及附件渲染
- 无图模式延迟加载正文图片并保留表情显示
- 多账号 Cookie 隔离和本地会话存储
- 板块收藏合并、主题收藏夹接口、子板块偏好和北京时间签到策略
- 回复表单预检、锁帖错误识别、单次提交及写请求不重试
- 楼层评价、主题投票、主题评分、消息未读策略、通知目标、主题分页和分享入口
- 运行日志敏感凭据脱敏
