...
SNGA 是一个使用 SwiftUI 构建的原生 macOS NGA 论坛客户端，当前版本为 1.8.2。它支持多账号登录、论坛浏览、话题互动、消息通知与常用资讯工具。
...
- 主题改为每套一张完整调色板，不再由背景色和强调色两个值推导；卡片不再比窗口底色更暗，明亮与 NGA 暖金下尤其明显
- 引用独立成块：抬头压成小字，被引正文单独一行，不再和抬头粘成一段
- 热点回复的标记跟着主题强调色走，午夜蓝等冷色主题下不再出现一块橙色
- 未读消息徽章和资讯热榜序号的文字跟着主题走，深色、午夜蓝和自定义主题下不再是几乎看不清的白字
- 分页条的翻页键收进分段控件，页码就地编辑，回车跳转
- 搜索框描边加重到看得清，原先几乎和背景融在一起
- 骨架屏、磁贴、分隔线等改用主题颜色，不再在暖色主题上扫过一道灰
- 正文里的图片支持右键菜单，复制图片、复制地址、另存为和在浏览器中打开，和放大预览时一致
...
- 跟随系统、明亮、深色、NGA 暖金、午夜蓝及自定义共六套主题，无图模式、运行日志和资讯热榜小工具
