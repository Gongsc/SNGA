...
SNGA 是一个使用 SwiftUI 构建的原生 macOS NGA 论坛客户端，当前版本为 1.8.0。它支持多账号登录、论坛浏览、话题互动、消息通知与常用资讯工具。
...
- 仅含文字、样式、表情和引用的楼层改用原生渲染，不再为每层创建网页视图，话题打开更快、占用更低
- 楼层改为按需加载，长话题的滚动更流畅
- 「Reply to」形式的回复现在会直接显示被引用楼层的内容
- 话题正文的解析与渲染去掉大量重复计算，翻页和主题切换更跟手
- 修复未登录时用户中心底部工具栏位置错误的问题
