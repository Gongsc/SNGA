...
- 楼层末尾会显示作者的签名，和正文之间隔一条分割线。NGA 的签名跟着话题页一起下发，和正文走同一条渲染管线；NodeSeek 的话题页里没有签名，显示的是资料里的个人简介 —— 那份资料本来就要为 IP 属地问一次，不额外增加请求。可以在「设置 → 浏览 → 在话题中显示签名」里关掉
- 用户中心里那一栏按站点自己的说法称呼：NGA 是「签名」，NodeSeek 是「个人简介」
...
| 楼层签名 | 跟着话题页下发，排版照原样显示（站点对未登录用户不下发） | 站点不给楼层签名，改用资料里的个人简介（纯文本） |
