...
SNGA 是一个使用 SwiftUI 构建的原生 macOS NGA 论坛客户端，当前版本为 1.8.1。它支持多账号登录、论坛浏览、话题互动、消息通知与常用资讯工具。
...
- 支持匿名用户和匿名话题，在用户名之后、话题标题之前显示匿名标记
- 楼层显示管理处罚导致的折叠提示，以及发出之后的改动时间
- 话题图片改用原生渲染并限制解码尺寸，含大量截图或超长图片的话题不再卡顿
- 楼层改回整页加载，修复长话题滚动时高度反复跳动、首楼被截断和回不到顶部的问题
- 修复带表情的楼层高度算错，正文越出楼层边框的问题
- 修复嵌套引用把 [quote] 标记漏进正文的问题
- 修复「Reply to」形式的回复把正文并进引用标题的问题
- 移除楼层作者下方无法点击的个人版面标签
