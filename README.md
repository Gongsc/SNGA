...
各站的差别举几个例子：NGA 有子版面、话题评分和收藏夹分组；NodeSeek 的楼层表态里有「加鸡腿」，要花鸡腿且撤不回来；V2EX 没有主题全文搜索（站点自己的搜索框只能找节点），它的「感谢」会花掉 10 个铜币、同样撤不回来，所以点之前会先问一次。
...
- V2EX 没有主题全文搜索、没有站内私信，正文里的用户链接会交给浏览器打开（站点的用户地址里是用户名，换成编号要额外请求一次）
- 不提供离线全文归档和自动更新
