...
- 支持楼层点赞、点踩及数量展示
- 支持图片、表情、表格、列表、章节标题、上标、文字颜色、分隔线、代码、引用和折叠内容
- 支持 NGA 带列宽参数的表格，并按行内权重计算实际列宽
...
- 论坛消息按 NGA 网页分为“短消息”和“提醒信息”
- 短消息与提醒信息均展示已读/未读状态，打开消息后自动标记为已读
- 提醒信息使用账号级本地状态保留已读结果，避免轮询接口提前清除服务端未读标记
- 支持消息分页加载，以及回复、引用和 @ 提醒类型识别
- 支持查看私信详情并回复已有私信
...
- NGA 表情在深色模式下使用浅色衬底，避免深色图案与背景混在一起
...
- 富文本、列表、折叠内容、表情、附件、章节标题、上标及安全颜色渲染
- 带参数表格解析与列宽权重换算
- 多账号 Cookie 隔离和本地会话存储
- 收藏合并、子板块偏好和北京时间签到策略
- 回复表单预检、单次提交及写请求不重试
- 短消息/提醒分类、已读状态、通知目标、主题分页和分享入口
- 主题列表与主题内容底部工具栏入口
